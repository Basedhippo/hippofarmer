...
/**
 * @title RoidHippos Non-Fungible Token
 * @author Basedhippodev (@BasedHippoDev)
 */
contract RoidHippos is TRC721, TRC721Enumerable, TRC721MetadataMintable {

    using Strings for uint256;

    uint256 public legendaryMaxSupply = 888;
    uint256 public poodengMaxSupply = 8888;
    uint256 public legendaryMintCost = 888888888; // 888 TRX per legendary token
    uint256 public poodengMintCost = 444444444; // 444 TRX per poodeng token
    uint256 public initialSteroidCost = 699420000; // 699.42 TRX per steroid
    ...
    constructor() public TRC721Metadata("RoidHippo", "ROIDHIPPO") {
        _setBaseURI("https://gateway.btfs.io/btfs/");
    }

    function safePublicMint(address _to, uint256[] memory _tokenId, bool isLegendary) public payable {
        require(_to != address(0x0), "Minting to the null address is not allowed");
        require(_tokenId.length > 0, "No tokens to mint");
        require(_tokenId.length < 5, "Only a maximum of 4 tokens per person is allowed");
        
        uint256 mintCost = isLegendary ? legendaryMintCost : poodengMintCost;
        require(msg.value >= mintCost * _tokenId.length, "Insufficient TRX for minting");
    ...
    function preMint(address _to, uint256[] memory _tokenId, bool isLegendary) public onlyOwner {
        require(_to != address(0x0), "Minting to the null address is not allowed");
    ...
    function mintSteroids(address _to, uint256 amount) public onlyOwner {
        require(amount <= steroidSupply, "Exceeds available steroid supply");
        steroidSupply -= amount;
        // Logic to distribute steroids to _to
    }

    function breedHippos(uint256 parent1Id, uint256 parent2Id, address _to) public payable {
        require(msg.value >= breedingDungCost, "Insufficient TRX for breeding");
        // Logic to breed two hippos and mint a new token to _to
    }
}
